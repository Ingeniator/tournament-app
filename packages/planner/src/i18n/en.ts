import type { Translations } from '@padel/common';

const en: Translations = {
  // HomeScreen
  'home.title': 'Tournament Planner',
  'home.loggedInAs': 'Logged in as',
  'home.editName': 'Edit name',
  'home.save': 'Save',
  'home.cancel': 'Cancel',
  'home.namePrompt': 'Set your name to get started',
  'home.namePromptSub': '(will be shown as tournament owner name)',
  'home.namePlaceholder': 'Group Name / Your name',
  'home.saving': 'Saving...',
  'home.tournamentName': 'Tournament name',
  'home.tournamentNamePlaceholder': 'Tournament name',
  'home.creating': 'Creating...',
  'home.createTournament': 'Create Tournament',
  'home.or': 'or',
  'home.codePlaceholder': 'ABCDEF',
  'home.codeMustBe6': 'Code must be 6 characters',
  'home.joining': 'Joining...',
  'home.join': 'Join',
  'home.joinWithCode': 'Join with Code',
  'home.myTournaments': 'My Tournaments',
  'home.noTournamentsCreated': 'No tournaments created yet',
  'home.registeredTournaments': 'Registered Tournaments',
  'home.noTournamentsJoined': 'No tournaments joined yet',
  'home.listingsError': 'Could not load tournaments. Please check your connection.',
  'home.tournamentNotFound': 'Tournament not found',
  'home.failedCreate': 'Failed to create tournament',
  'home.expired': 'Expired',
  'home.completed': 'Completed',
  'home.groupTournaments': 'Group Tournaments',
  'home.playerMode': 'Play',
  'home.organizerMode': 'Organize',
  'home.joinNotFound': 'No tournament or event found with this code',
  'home.myEvents': 'My Events',
  'home.importTournament': 'Import',
  'home.importFailed': 'Invalid tournament data',
  'home.signInWithGoogle': 'Sign in with Google',
  'home.linking': 'Signing in...',

  // Footer & Options
  'footer.freeOpenSource': 'Free & open source',
  'footer.madeWithCare': 'Made with care',
  'footer.supportUs': 'Support us',
  'footer.sendFeedback': 'Send feedback',
  'footer.options': 'Personalize',
  'options.title': 'Personalize',
  'options.language': 'Language',

  // OrganizerScreen
  'organizer.back': 'Back',
  'organizer.renameTournament': 'Rename tournament',
  'organizer.by': 'by {name}',
  'organizer.modeQuickPlay': 'Quick Play',
  'organizer.modeShare': 'Share & Invite',
  'organizer.playersSimple': 'Players ({confirmed} / {capacity})',
  'organizer.shareWithPlayers': 'Share with Players',
  'organizer.playersEnterCode': 'Players enter this code to register',
  'organizer.copyLink': 'Copy Link',
  'organizer.linkCopied': 'Link copied!',
  'organizer.codeCopied': 'Code copied!',
  'organizer.failedCopy': 'Failed to copy',
  'organizer.players': 'Players ({confirmed} / {capacity}{reserve})',
  'organizer.playersCount': 'Players ({count})',
  'organizer.reserveSuffix': ' + {count} reserve',
  'organizer.noPlayersYet': 'No players registered yet',
  'organizer.playerNamePlaceholder': 'Player name or paste a list',
  'organizer.addPlayer': '+ Add',
  'organizer.linkProfile': 'Link profile',
  'organizer.linkProfileTitle': 'Link {name}',
  'organizer.aliasLabel': 'Display name (alias)',
  'organizer.aliasPlaceholder': 'Name used during the game',
  'organizer.telegramUsername': 'Telegram username',
  'organizer.telegramPlaceholder': '@username',
  'organizer.saveLink': 'Save',
  'organizer.removeLink': 'Unlink Telegram',
  'organizer.partnerSection': 'Partner',
  'organizer.partnerName': 'Select partner…',
  'organizer.partnerTelegram': 'Partner telegram (optional)',
  'organizer.newPlayer': '+ New player',
  'organizer.newPlayerName': 'Player name',
  'organizer.removePartner': 'Remove partner',
  'organizer.addedBy': 'Added by {name}',
  'organizer.reserve': 'reserve',
  'organizer.markCancelled': 'Mark as cancelled',
  'organizer.markConfirmed': 'Mark as confirmed',
  'organizer.removePlayer': 'Remove player',
  'organizer.actions': 'Actions',
  'organizer.selectClub': 'Select club',
  'organizer.selectRank': 'Select rank',
  'organizer.settings': 'Settings',
  'organizer.whenWhere': 'When & Where',
  'organizer.formatAndCourts': 'Format & Courts',
  'organizer.details': 'Details',
  'organizer.dateTime': 'Date & time',
  'organizer.date': 'Date',
  'organizer.time': 'Time',
  'organizer.durationMin': 'Duration (min)',
  'organizer.place': 'Place',
  'organizer.placePlaceholder': 'Venue / location',
  'organizer.groupChat': 'Group chat',
  'organizer.groupChatPlaceholder': 'https://t.me/...',
  'organizer.description': 'Description',
  'organizer.descriptionPlaceholder': 'Price, skill level, rules, etc.',
  'organizer.format': 'Format',
  'organizer.formatInfo': 'Format info',
  'organizer.formatAmericano': 'Americano',
  'organizer.formatTeamAmericano': 'Team Americano',
  'organizer.formatMexicano': 'Mexicano',
  'organizer.formatTeamMexicano': 'Team Mexicano',
  'organizer.formatMixicano': 'Mixicano',
  'organizer.formatMixedAmericano': 'Mixed Americano',
  'organizer.mixedAmericanoDesc': 'Cross-group random partners',
  'organizer.formatKingOfTheCourt': 'King of the Court',
  'organizer.formatClubAmericano': 'Club Americano',
  'organizer.americanoDesc': 'Americano \u2014 Random partner rotation each round. Individual standings.',
  'organizer.teamAmericanoDesc': 'Team Americano \u2014 Fixed teams for the entire tournament. Team standings.',
  'organizer.mexicanoDesc': 'Mexicano \u2014 After round 1, pairings based on standings \u2014 top player pairs with bottom, keeping matches competitive.',
  'organizer.teamMexicanoDesc': 'Team Mexicano \u2014 Fixed teams, but matchups based on standings. Keeps games competitive.',
  'organizer.mixicanoDesc': 'Mixicano \u2014 Like Mexicano, but every team must have one player from each group. Individual standings.',
  'organizer.kingOfTheCourtDesc': 'King of the Court \u2014 Court hierarchy with promotion/relegation. Winners move up, losers move down.',
  'organizer.clubAmericanoDesc': 'Club Americano \u2014 Clubs of 2\u20133 pairs compete. Partners are always from the same club. Club standings.',
  'organizer.duration': 'Duration (minutes)',
  'organizer.durationPlaceholder': '120',
  'organizer.courts': 'Courts ({count})',
  'organizer.addCourt': '+ Add',
  'organizer.courtBonusAuto': 'bonus pts',
  'organizer.extraSpots': 'Extra spots',
  'organizer.totalSpots': 'Total spots: {total} ({courts} \u00d7 4{extra})',
  'organizer.extraSuffix': ' + {count}',
  'organizer.spotsFilled': 'Only {confirmed} of {capacity} spots filled.',
  'organizer.fillsCourts': ' Need {remaining} more to reach planned capacity, but enough to fill {courts}.',
  'organizer.courtsSingle': 'the court',
  'organizer.courtsMultiple': 'all {count} courts',
  'organizer.notFullGames': ' Some courts won\'t have full games.',
  'organizer.notEnoughPlayers': 'Not enough players: {count} confirmed, need at least {needed}.',
  'organizer.duplicateNames': 'Duplicate names: {names}. Same person registered twice?',
  'organizer.unassignedGroups': '{count} player(s) not assigned to a group — will be assigned automatically',
  'organizer.validationNeedsPartner': '{count} player(s) still need a partner',
  'organizer.validationMinPlayers': 'At least 4 players are needed to start',
  'organizer.validationEvenPlayers': 'Team formats require an even number of players',
  'organizer.validationUnassignedClubs': '{count} player(s) not assigned to a club',
  'organizer.validationTooManyCourts': 'Too many courts: {courts} courts but only {max} can be used',
  'organizer.letsPlay': 'Let\'s play',
  'organizer.copyForDevice': 'Copy for Another Device',
  'organizer.jsonCopied': 'Tournament JSON copied!',
  'organizer.export': 'Export',
  'organizer.import': 'Import',
  'organizer.copyData': 'Copy to Clipboard',
  'organizer.exportFile': 'Download File',
  'organizer.importFromClipboard': 'Paste from Clipboard',
  'organizer.loadFile': 'Load File',
  'organizer.exportCopied': 'Tournament export copied!',
  'organizer.importConfirm': 'Import "{name}" with {count} players? This creates a new tournament.',
  'organizer.deleteTournament': 'Delete Tournament',
  'organizer.deleteConfirm': 'Delete this tournament? This cannot be undone.',
  'organizer.completed': 'Tournament Completed',
  'organizer.completedOn': 'Completed on {date}',
  'organizer.undoComplete': 'Reopen Tournament',
  'organizer.viewResults': 'View Results',
  'organizer.loadingResults': 'Loading results...',
  'organizer.noBackupData': 'Results not available',
  'organizer.reopenTitle': 'Reopen Tournament?',
  'organizer.reopenWarning': 'Reopening will erase all saved scores and results. The tournament will return to registration and you\'ll need to start fresh.',
  'organizer.reopenConfirm': 'Reopen',
  'organizer.groupLabels': 'Group labels',
  'organizer.groupLabelAPlaceholder': 'Left side player',
  'organizer.groupLabelBPlaceholder': 'Right side player',
  'organizer.rankLabels': 'Rank labels',
  'organizer.rankLabelPlaceholder': 'Slot {num} (e.g. Pro)',
  'organizer.clubs': 'Clubs ({count})',
  'organizer.addClub': '+ Add',
  'organizer.noClub': 'Add at least 2 clubs',
  'organizer.courts_section': 'Courts',
  'organizer.formatSection': 'Format',
  'organizer.modesSubtitle': 'Modes',
  'organizer.matchSettings': 'Match Settings',
  'organizer.maldicionesEnabled': '\uD83C\uDFAD Maldiciones del Padel',
  'organizer.maldicionesHint': 'Teams receive curse cards to disrupt opponents during matches',
  'organizer.chaosLevel': 'Chaos Level',
  'organizer.chaosLite': 'Lite (green cards only)',
  'organizer.chaosMedium': 'Medium (green + yellow)',
  'organizer.chaosHardcore': 'Hardcore (all cards)',
  'organizer.numberOfRounds': 'Number of rounds',
  'organizer.gamesPerMatch': 'Games per match',
  'organizer.setsPerMatch': 'Sets per match',
  'organizer.pointsPerMatch': 'Points per match',
  'organizer.scoringMode': 'Scoring',
  'organizer.scoringPoints': 'Points',
  'organizer.scoringGames': 'Games (15/30/40)',
  'organizer.scoringSets': 'Sets',
  'organizer.scoringTimed': 'Timed',
  'organizer.minutesPerRound': 'Minutes per round',
  'organizer.estimatedDuration': 'Estimated duration: ',
  'organizer.estimateBreakdown': '({rounds} rounds \u00d7 {minutes} min per round)',
  'organizer.recommendedRounds': '{rounds} rounds for {players} players on {courts} court(s)',
  'organizer.clubFixedRounds': '{rounds} rounds for {clubs} clubs (round-robin)',
  'organizer.recommendedPoints': 'Max: {points} points per match',
  'organizer.recommendedGames': 'Max: {games} games per match',
  'organizer.recommendedSets': 'Max: {sets} sets per match',
  'organizer.clubPanel': 'Clubs',
  'organizer.clubUnassigned': 'Unassigned',
  'organizer.gamesModeSuggestion': 'With only {rounds} rounds, games mode (15/30/40 scoring) fills the target duration better',
  'organizer.somePlayersWontPlay': 'Some players won\'t play',
  'organizer.playersPerRound': 'Only {playing} of {total} players play each round. Need at least {minRounds} rounds for everyone to participate.',
  'organizer.mayExceed': 'May exceed {duration}',
  'organizer.fitWithin': 'To fit within {duration}, try',
  'organizer.pointsSuggestion': '{points} points per match or ',
  'organizer.roundsSuggestion': '{rounds} rounds',
  'organizer.setsSuggestion': '{sets} sets per match or ',
  'organizer.gamesSuggestion': '{games} games per match or ',
  'organizer.timedSuggestion': '{rounds} rounds \u00d7 {minutes} min',
  'organizer.recommendedTimedPair': 'Recommended: {rounds} rounds \u00d7 {minutes} min',
  'organizer.unequalSitOuts': 'Unequal sit-outs',
  'organizer.sitOutBody': 'With {rounds} rounds, {sitOuts} player(s) sit out each round \u2014 sit-outs cannot be split equally across {players} players.',
  'organizer.trySitOut': 'Try {below} or {above} rounds for equal sit-outs.',
  'organizer.trySitOutSingle': 'Try {rounds} rounds for equal sit-outs.',
  'organizer.rankCapacityWarningTitle': 'Not enough capacity for ranks',
  'organizer.rankCapacityWarningBody': 'Club-ranked requires at least {minCapacity} players ({courts} courts). Current capacity is {capacity}. Add more courts or select other tournament format.',
  'organizer.startDelegate': 'Who can start',
  'organizer.startDelegateOnlyMe': 'Only me',
  'organizer.startDelegateTelegram': 'Telegram user',
  'organizer.startDelegateTelegramPlaceholder': '@username',
  'organizer.captainMode': 'Captain mode',
  'organizer.captainModeHint': 'Club captains pair their players and manage lineups each round',
  'organizer.clubCaptain': 'Club captain',
  'organizer.noCaptain': 'No captain',
  'organizer.setCaptain': 'Set captain',
  'organizer.captainInstructions': 'As a captain, pair players from your club and approve or reject their pairs.',
  'organizer.captainUnapproved': '{count} player(s) awaiting captain approval. Set yourself as captain or share the captain link to approve lineups.',
  'organizer.sectionPlaying': 'Playing ({count})',
  'organizer.sectionReserve': 'Reserve ({count})',
  'organizer.sectionRegistered': 'Registered ({count})',
  'organizer.sectionNeedsPartner': 'Needs partner ({count})',
  'organizer.sectionCancelled': 'Cancelled ({count})',

  // JoinScreen
  'join.back': 'Back',
  'join.date': 'Date',
  'join.duration': 'Duration',
  'join.place': 'Place',
  'join.organizer': 'Organizer',
  'join.groupChat': 'Group chat',
  'join.joinGroupChat': 'Join group chat',
  'join.description': 'Description',
  'join.onReserveList': 'You\'re on the reserve list',
  'join.reservePosition': 'Reserve #{position} \u2014 you\'ll move up if someone cancels',
  'join.confirmed': 'You\'re confirmed!',
  'join.confirmedHint': 'You\'ll appear in the player list below',
  'join.cancelled': 'You\'ve cancelled',
  'join.cancelledHint': 'You can confirm again at any time',
  'join.registeredAs': 'You registered as',
  'join.editName': 'Edit name',
  'join.save': 'Save',
  'join.updating': 'Updating...',
  'join.cancelParticipation': 'Cancel participation',
  'join.confirmParticipation': 'Confirm participation',
  'join.addToCalendar': 'Add to Calendar',
  'join.joinAs': 'Join this tournament as',
  'join.enterName': 'Enter your name',
  'join.registering': 'Registering...',
  'join.register': 'Register',
  'join.players': 'Players ({confirmed} / {capacity})',
  'join.spotsLeft': '{count} spot{s} left',
  'join.fullReserve': 'Full \u00b7 {count} on reserve list',
  'join.full': 'Full',
  'join.courtInfo': '{courts} court{s} \u00d7 4{extra}',
  'join.noPlayersYet': 'No players yet. Be the first!',
  'join.reserve': 'reserve',
  'join.duplicateConfirm': '"{name}" is already registered. Is that a different person?',
  'join.reserveToast': 'You\'re on the reserve list \u2014 we\'ll bump you up if a spot opens',
  'join.inWithCalendar': 'You\'re in! Add it to your calendar so you don\'t forget',
  'join.inSimple': 'You\'re in! See you on the court',
  'join.registerFailed': 'Could not register, please try again',
  'join.participationCancelled': 'Participation cancelled',
  'join.welcomeBackCalendar': 'Welcome back! Add it to your calendar so you don\'t forget',
  'join.welcomeBack': 'Welcome back! You\'re confirmed',
  'join.updateFailed': 'Could not update, please try again',
  'join.letsPlay': 'Let\'s play',
  'join.startTournament': 'Start Tournament',
  'join.copyForDevice': 'Copy for Another Device',
  'join.jsonCopied': 'Tournament JSON copied!',
  'join.failedCopy': 'Failed to copy',
  'join.copied': 'Copied to clipboard',
  'join.exportPairs': 'Copy pairs',
  'join.completed': 'This tournament has been completed',
  'join.completedOn': 'Completed on {date}',
  'join.format': 'Format',
  'join.selectGroup': 'Select your group',
  'join.selectClub': 'Select your club',
  'join.selectRank': 'Select your rank',
  'join.partnerLabel': 'Preferred partner',
  'join.partnerName': 'Partner name',
  'join.partnerTelegram': 'Partner telegram (optional)',
  'join.partnerTaken': '{name} is already paired with another player',
  'join.partnerDifferentClub': '{name} is in a different club',
  'join.partnerDifferentRank': '{name} is in a different rank',
  'join.partnerSameGroup': '{name} is in the same group — partner must be from the other group',
  'join.changeBreaksLink': 'This change will disconnect your partner link with {name}. Continue?',
  'join.edit': 'Edit',
  'join.needsPartner': 'Needs a partner',
  'join.needsPartnerHint': 'Add a partner to secure your spot',
  'join.needsPartnerHintCaptain': 'Help your captain to pair you with a partner',
  'join.awaitingCaptain': 'Awaiting captain approval',
  'join.awaitingCaptainHint': 'Your club captain will review your pair',
  'join.yourCaptain': 'Your captain:',
  'join.captainUnassigned': 'Unassigned',
  'join.approve': 'Approve',
  'join.reject': 'Reject',
  'join.pairSlots': '{pairs} pair slots',
  'join.myClub': '{name}',
  'join.allPlayers': 'All Players',

  // SupportOverlay
  'support.title': 'Support Us',
  'support.helpKeepFree': 'Help keep it free',
  'support.description': 'Tournament Manager is free and open source. Your support helps us keep it that way.',
  'support.yourName': 'Your name',
  'support.leaveMessage': 'Leave a message (optional)',
  'support.support': 'Support',
  'support.thankYou': 'Thank you!',
  'support.testingFlow': 'We\'re testing this support flow. Real payment options coming soon!',
  'support.supporters': 'Supporters',
  'support.raised': '{amount} raised',
  'support.beFirst': 'Be the first!',

  // Common (shared component strings)
  'feedback.title': 'Send Feedback',
  'feedback.placeholder': 'Bug report, feature request, or just say thanks...',
  'feedback.sending': 'Sending...',
  'feedback.send': 'Send',
  'error.title': 'Something went wrong',
  'error.reload': 'Reload',

  // Start Warning
  'startWarning.title': 'Tournament Already Started',
  'startWarning.alreadyStarted': '{name} has already started this tournament on their device.',
  'startWarning.alreadyStartedSelf': 'You have already started this tournament.',
  'startWarning.restartWillErase': 'Restarting will erase all scores entered so far. If you want to continue the existing tournament, go to the scoring app instead.',
  'startWarning.onePersonOnly': 'Only one person should enter results \u2014 scores are stored locally on that person\'s device, not online.',
  'startWarning.dataLost': 'If that person\'s data was lost, you can proceed and start fresh.',
  'startWarning.continueTournament': 'Continue Tournament',
  'startWarning.proceedAnyway': 'Start Anyway',

  // FormatPicker
  'format.browseMode': 'Browse',
  'format.wizardMode': 'Help me choose',
  'format.categorySocial': 'Social',
  'format.categoryCompetitive': 'Competitive',
  'format.categoryTeam': 'Team',
  'format.categoryClub': 'Club',
  'format.americano': 'Americano',
  'format.americanoDesc': 'Random rotating partners and opponents',
  'format.mixedAmericano': 'Mixed Americano',
  'format.mixedAmericanoDesc': 'Cross-group random partners',
  'format.mexicano': 'Mexicano',
  'format.mexicanoDesc': 'Rotating partners, standings-based opponents',
  'format.mixicano': 'Mixicano',
  'format.mixicanoDesc': 'Cross-group pairs, standings-based opponents',
  'format.kingOfTheCourt': 'King of the Court',
  'format.kingOfTheCourtDesc': 'Court promotion/relegation system',
  'format.teamAmericano': 'Team Americano',
  'format.teamAmericanoDesc': 'Fixed teams, random opponents',
  'format.teamMexicano': 'Team Mexicano',
  'format.teamMexicanoDesc': 'Fixed teams, standings-based opponents',
  'format.mixedTeamAmericano': 'Mixed Team Americano',
  'format.mixedTeamAmericanoDesc': 'Cross-group fixed teams, random opponents',
  'format.mixedTeamMexicano': 'Mixed Team Mexicano',
  'format.mixedTeamMexicanoDesc': 'Cross-group fixed teams, standings-based opponents',
  'format.mixedKingOfTheCourt': 'Mixed King of the Court',
  'format.mixedKingOfTheCourtDesc': 'Cross-group pairs, court promotion/relegation',
  'format.clubAmericano': 'Club Americano',
  'format.clubAmericanoDesc': 'Random partners within club, inter-club matches',
  'format.clubRanked': 'Club Ranked',
  'format.clubRankedDesc': 'Ranked slot inter-club matches',
  'format.clubTeamAmericano': 'Club Team Americano',
  'format.clubTeamAmericanoDesc': 'Random inter-club pair matchups',
  'format.clubMexicano': 'Club Mexicano',
  'format.clubMexicanoDesc': 'Rotating partners within club, standings-based opponents',
  'format.clubTeamMexicano': 'Club Team Mexicano',
  'format.clubTeamMexicanoDesc': 'Standings-based inter-club matchups',
  'format.wizardStep1': 'Partners',
  'format.wizardStep2': 'Opponents',
  'format.wizardStep3': 'Options',
  'format.wizardRotating': 'Rotating',
  'format.wizardFixed': 'Fixed teams',
  'format.wizardRandom': 'Random',
  'format.wizardStandings': 'By standings',
  'format.wizardPromotion': 'Court promotion',
  'format.wizardPromotionDisabled': 'Only available with rotating partners',
  'format.wizardCrossGroup': 'Cross-group pairs',
  'format.wizardClub': 'Club competition',
  'format.wizardResult': 'Recommended format',
  'format.wizardNotYet': 'This combination is not available yet',

  // Event
  'event.back': 'Back',
  'event.myEvents': 'My Events',
  'event.noEvents': 'No events created yet',
  'event.createEvent': 'Create Event',
  'event.createTitle': 'New Event',
  'event.name': 'Event name',
  'event.namePlaceholder': 'Club Championship March 2026',
  'event.date': 'Date',
  'event.create': 'Create Event',
  'event.creating': 'Creating...',
  'event.createFailed': 'Failed to create event',
  'event.notFound': 'Event not found',
  'event.status.draft': 'Draft',
  'event.status.active': 'Live',
  'event.status.completed': 'Completed',
  'event.tournaments': 'Tournaments ({count})',
  'event.noTournaments': 'No tournaments linked yet',
  'event.addTournament': 'Add Tournament',
  'event.myTournaments': 'My Tournaments',
  'event.linkByCode': 'Link by code',
  'event.linkPlaceholder': 'Tournament code',
  'event.link': 'Link',
  'event.unlink': 'Unlink',
  'event.unlinkConfirm': 'Remove this tournament from the event?',
  'event.alreadyLinked': 'Tournament is already linked',
  'event.tournamentLinked': 'Tournament linked!',
  'event.linkFailed': 'Failed to link tournament',
  'event.players': 'players',
  'event.registered': '{count} registered',
  'event.spotsOpen': '{count} spots open',
  'event.weight': 'Weight',
  'event.liveStandings': 'Live Standings',
  'event.finalStandings': 'Final Standings',
  'event.noStandings': 'No results to show yet. Link tournaments with completed matches.',
  'event.liveClubStandings': 'Live Club Standings',
  'event.finalClubStandings': 'Club Standings',
  'event.delete': 'Delete Event',
  'event.deleteConfirm': 'Delete this event? This does not delete any linked tournaments.',
  'event.col.name': 'Player',
  'event.col.club': 'Club',
  'event.col.pts': 'Pts',
  'event.col.mp': 'MP',
  'event.col.w': 'W',
  'event.col.members': 'Players',
  'event.col.diff': '+/-',
  'event.shareWithPlayers': 'Share with Players',
  'event.shareHint': 'Players use this link to view the event and join tournaments',
  'event.copyLink': 'Copy Link',
  'event.linkCopied': 'Link copied!',
  'event.codeCopied': 'Code copied!',
  'event.description': 'Description',
  'event.descriptionPlaceholder': 'Event details, rules, schedule, etc.',
  'event.export': 'Export',
  'event.import': 'Import',
  'event.exportCopied': 'Event export copied!',
  'event.importConfirm': 'Import event "{name}" with {count} tournament(s)? This creates a new event.',

  // EventJoinScreen
  'eventJoin.join': 'Join',
  'eventJoin.open': 'Open',
  'eventJoin.edit': 'Edit',

  // Breakdown
  'breakdown.spotsOpen': '{count} spots open',
  'breakdown.hurrySpots': 'Hurry! Only {count} spots left',
  'breakdown.lastSpot': 'Last spot!',
  'breakdown.full': 'Full',
  'breakdown.clubNeedsMore': '{club} needs {count} more',
  'breakdown.clubNeedsMoreCaptain': '{club} needs more players',
  'breakdown.noRankPlayers': '{club} {rank} needs {count} more',
  'breakdown.noRankPlayersCaptain': '{club} needs more {rank} players',
  'breakdown.groupNeedsMore': '{group} needs {count} more',
  'breakdown.paired': 'Paired',
  'breakdown.captainMode': 'Captain Mode',
  'breakdown.maldiciones': 'Maldiciones',

  // Team Pairing (shared with runner)
  'teams.title': 'Teams',
  'teams.subtitle': '{playerCount} players \u2192 {teamCount} teams',
  'teams.hint': 'Tap two players to swap them between teams',
  'teams.hintClub': 'Tap two players from the same club to swap them',
  'teams.shuffle': 'Shuffle Teams',
  'teams.start': 'Start Tournament',
  'teams.groupsAutoAssigned': '{count} player(s) had no group — assigned automatically',

  // Planner App
  'app.connectionError': 'Connection Error',
  'app.retry': 'Retry',
  'app.firebaseRequired': 'Firebase Setup Required',
  'app.firebaseInstructions': 'Create a .env file in packages/planner/ with your Firebase config.',
  'app.firebaseSeeExample': 'See .env.example for the required variables.',

  // Auth errors
  'auth.connectionFailed': 'Could not connect. Check your internet and try again.',
  'auth.googleSignInFailed': 'Failed to sign in with Google',
  'auth.googleLinkFailed': 'Failed to link Google account',
};

export default en;
