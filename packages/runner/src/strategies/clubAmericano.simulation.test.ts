...
describe('clubAmericano simulation', () => {
    ...
    it('structural invariants hold', () => {
      for (let t = 0; t < TRIALS; t++) {
        const rounds = simulateDynamic(
          clubAmericanoStrategy, players, config, numRounds,
          { teams, clubs },
        );
  ...
  describe('3 clubs × 4p / 2c / 3r', () => {
    ...
    it('sit-out spread ≤ 1 across teams', () => {
      for (let t = 0; t < TRIALS; t++) {
        ...
        const stats = analyzeTeamSchedule(teams, rounds);
        // With 3 clubs, 1 sits out per round. Each club byes once in 3 rounds.
  ...
  describe('3 clubs × 6p / 3c / 6r (larger clubs)', () => {
    ...
    it('game spread is reasonable', () => {
      for (let t = 0; t < TRIALS; t++) {
        ...
        const stats = analyzeTeamSchedule(teams, rounds);
        // With 3 clubs (3 pairs each), 1 club sits out per round.
        // 3 courts can only hold 3 matches, but a fixture has 3 pairs × 3 pairs = max 3 matches.
        // All pairs from 2 fixture clubs should play each round they're active.
