...
/** Check that every team in every match has one group-A and one group-B player */
...
describe('mixicano strategy', () => {
  describe('strategy properties', () => {
    it('is a dynamic strategy', () => {
      expect(mixicanoStrategy.isDynamic).toBe(true);
    });

    it('does not have fixed partners', () => {
      expect(mixicanoStrategy.hasFixedPartners).toBe(false);
    });
  });
  ...
  describe('validateSetup', () => {
    it('requires at least 4 players', () => {
      ...
      const errors = mixicanoStrategy.validateSetup(players, makeConfig(1));
    ...
    it('requires at least 1 court', () => {
      ...
      const errors = mixicanoStrategy.validateSetup(makePlayers(2, 2), config);
    ...
    it('rejects too many courts for the group sizes', () => {
      // 2A + 2B → max 1 court (min(2,2)/2 = 1)
      const errors = mixicanoStrategy.validateSetup(makePlayers(2, 2), makeConfig(2));
      expect(errors.some(e => e.includes('Too many courts'))).toBe(true);
    });

    it('passes for valid setup', () => {
      const errors = mixicanoStrategy.validateSetup(makePlayers(4, 4), makeConfig(2));
      expect(errors).toEqual([]);
    });

    it('passes for 2A + 2B on 1 court', () => {
      const errors = mixicanoStrategy.validateSetup(makePlayers(2, 2), makeConfig(1));
      expect(errors).toEqual([]);
    });
  });
  ...
  describe('validateWarnings', () => {
    it('warns about unequal groups', () => {
      const warnings = mixicanoStrategy.validateWarnings!(makePlayers(3, 2), makeConfig(1));
      expect(warnings.some(w => w.includes('unequal'))).toBe(true);
    });

    it('no warning for equal groups', () => {
      const warnings = mixicanoStrategy.validateWarnings!(makePlayers(3, 3), makeConfig(1));
  ...
  describe('validateScore', () => {
    it('accepts valid score', () => {
      expect(mixicanoStrategy.validateScore(
        { team1Points: 11, team2Points: 10 }, makeConfig(1),
      )).toBeNull();
    });

    it('rejects wrong total', () => {
      const err = mixicanoStrategy.validateScore(
  ...
  describe('generateSchedule', () => {
    it('generates exactly 1 round initially', () => {
      const { rounds } = mixicanoStrategy.generateSchedule(makePlayers(4, 4), makeConfig(2));
    ...
    it('every team has cross-group partners', () => {
      ...
      const { rounds } = mixicanoStrategy.generateSchedule(players, makeConfig(2));
    ...
    it('assigns all players to matches or sit-outs', () => {
      ...
      const { rounds } = mixicanoStrategy.generateSchedule(players, makeConfig(1));
    ...
    it('returns warning when sit-outs are needed', () => {
      const { warnings } = mixicanoStrategy.generateSchedule(makePlayers(3, 3), makeConfig(1));
      expect(warnings.some(w => w.includes('sit out'))).toBe(true);
    });

    it('returns no warnings when no sit-outs', () => {
      const { warnings } = mixicanoStrategy.generateSchedule(makePlayers(4, 4), makeConfig(2));
      expect(warnings).toHaveLength(0);
    });
  ...
  describe('generateAdditionalRounds', () => {
    ...
    it('uses standings-based pairing for subsequent rounds', () => {
      const players = makePlayers(4, 4);
      const config = makeConfig(2);
      const initial = mixicanoStrategy.generateSchedule(players, config);
      const scored = scoreAllMatches(initial.rounds);
  ...
  describe('calculateStandings', () => {
    it('returns standings for all players', () => {
      ...
      const standings = mixicanoStrategy.calculateStandings(tournament);
    ...
    it('awards sit-out compensation', () => {
      ...
      const tournament = {
        ...
        phase: 'in-progress' as const, players,
        rounds: [{
          id: 'r1', roundNumber: 1, sitOuts: ['a3', 'b3'],
          matches: [{
            id: 'm1', courtId: 'c1',
            team1: ['a1', 'b1'] as [string, string],
            team2: ['a2', 'b2'] as [string, string],
      ...
      const standings = mixicanoStrategy.calculateStandings(tournament);
      const a3 = standings.find(s => s.playerId === 'a3')!;
      // avg = (13 + 8) / 4 = 5.25, round = 5
      expect(a3.totalPoints).toBe(5);
    });
  });
