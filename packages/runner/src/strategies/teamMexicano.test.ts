...
describe('teamMexicano strategy', () => {
  describe('strategy properties', () => {
    it('is a dynamic strategy', () => {
      expect(teamMexicanoStrategy.isDynamic).toBe(true);
    });

    it('has fixed partners', () => {
      expect(teamMexicanoStrategy.hasFixedPartners).toBe(true);
    });
  });

  describe('validateSetup', () => {
    it('requires at least 4 players', () => {
      const errors = teamMexicanoStrategy.validateSetup(makePlayers(2), makeConfig(1));
      expect(errors.some(e => e.includes('4 players'))).toBe(true);
    });

    it('requires even number of players', () => {
      const errors = teamMexicanoStrategy.validateSetup(makePlayers(5), makeConfig(1));
    ...
    it('requires at least 1 court', () => {
      ...
      const errors = teamMexicanoStrategy.validateSetup(makePlayers(4), config);
    ...
    it('rejects too many courts for the number of teams', () => {
      // 4 players = 2 teams, need at most 1 court
      const errors = teamMexicanoStrategy.validateSetup(makePlayers(4), makeConfig(2));
      expect(errors.some(e => e.includes('Too many courts'))).toBe(true);
    });

    it('passes for valid setup', () => {
      const errors = teamMexicanoStrategy.validateSetup(makePlayers(8), makeConfig(2));
      expect(errors).toEqual([]);
    });

    it('passes for 4 players and 1 court', () => {
      const errors = teamMexicanoStrategy.validateSetup(makePlayers(4), makeConfig(1));
      expect(errors).toEqual([]);
    });
    ...
    it('returns multiple errors simultaneously', () => {
      ...
      const errors = teamMexicanoStrategy.validateSetup(makePlayers(3), config);
  ...
  describe('validateScore', () => {
    it('accepts valid score', () => {
      expect(teamMexicanoStrategy.validateScore(
        { team1Points: 11, team2Points: 10 }, makeConfig(1),
      )).toBeNull();
    });

    it('rejects wrong total', () => {
      const err = teamMexicanoStrategy.validateScore(
  ...
  describe('generateSchedule', () => {
    it('returns empty with no teams', () => {
      ...
      const { rounds, warnings } = teamMexicanoStrategy.generateSchedule(players, config, tournament);
    ...
    it('generates exactly 1 round initially', () => {
      ...
      const { rounds } = teamMexicanoStrategy.generateSchedule(players, config, tournament);
    ...
    it('all teams play or sit out each round', () => {
      ...
      for (const round of rounds) {
        const playingIds = round.matches.flatMap(m => [...m.team1, ...m.team2]);
        const allAccountedFor = new Set([...playingIds, ...round.sitOuts]);
    ...
    it('warns about sit-outs with odd number of teams', () => {
      ...
      const config = makeConfig(1);
      const tournament = makeTournament(players, teams, config);
      const { warnings } = teamMexicanoStrategy.generateSchedule(players, config, tournament);
      expect(warnings.some(w => w.includes('sit out'))).toBe(true);
    });

    it('no warnings when all teams play every round', () => {
      const players = makePlayers(4);
      const teams = makeTeams(players); // 2 teams, 1 court
      const config = makeConfig(1);
      const tournament = makeTournament(players, teams, config);
      const { warnings } = teamMexicanoStrategy.generateSchedule(players, config, tournament);
  ...
  describe('generateAdditionalRounds', () => {
    ...
    it('uses standings-based pairing for additional rounds', () => {
      const players = makePlayers(8);
      const teams = makeTeams(players);
      const config = makeConfig(2);
      const tournament = makeTournament(players, teams, config);
    ...
    it('returns empty when too many excluded', () => {
      const players = makePlayers(4);
      const teams = makeTeams(players);
      const config = makeConfig(1);
      const tournament = makeTournament(players, teams, config);
    ...
    it('sit-out entries contain player IDs from sitting-out teams', () => {
      ...
      for (const round of rounds) {
        if (round.sitOuts.length > 0) {
          expect(round.sitOuts).toHaveLength(2); // 1 team = 2 players
  ...
  describe('calculateStandings', () => {
    it('returns standings for all teams', () => {
      ...
      const config = makeConfig(1);
      const tournament = makeTournament(players, teams, config);
      const standings = teamMexicanoStrategy.calculateStandings(tournament);
    ...
    it('applies sit-out compensation', () => {
      ...
      // avg = (15 + 6) / 2 teams = 10.5 → rounds to 11
    ...
    it('excludes teams with unavailable players', () => {
      ...
      const teams = makeTeams(players);
      const config = makeConfig(1);
      players[0].unavailable = true; // t1 excluded
      const tournament = makeTournament(players, teams, config);
      const standings = teamMexicanoStrategy.calculateStandings(tournament);
