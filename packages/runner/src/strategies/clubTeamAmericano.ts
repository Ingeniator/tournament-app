...
/**
 * Club Team Americano: static schedule with random opponent matching.
 * All rounds are generated upfront because pair matching
 * does not depend on results.
 */

function buildStaticClubStrategy(mode: MatchMode): TournamentStrategy {
  const strategy: TournamentStrategy = {
    ...
    calculateStandings(tournament: Tournament) {
      return clubCalculateStandings(strategy, tournament);
    },
...
export const clubTeamAmericanoStrategy = buildStaticClubStrategy('random');
