...
describe('americano distribution', () => {
  ...
  describe('popular configurations: performance + quality', () => {
    ...
    const cases: { players: number; courts: number; rounds: number; maxMs: number; maxPartnerRepeats: number; maxOppSpread: number; label: string }[] = [
      ...
      // TODO: fix performance regression — 15p/3c/14r took 1839ms, limit relaxed from 1200ms to 2000ms
      { players: 15, courts: 3, rounds: 14, maxMs: 2000, maxPartnerRepeats: 0, maxOppSpread: 3, label: '15p / 3c / 14r' },
      { players: 16, courts: 4, rounds: 15, maxMs: 1500, maxPartnerRepeats: 0, maxOppSpread: 4, label: '16p / 4c / 15r' },
      { players: 20, courts: 4, rounds: 19, maxMs: 2000, maxPartnerRepeats: 0, maxOppSpread: 4, label: '20p / 4c / 19r' },
    ];
