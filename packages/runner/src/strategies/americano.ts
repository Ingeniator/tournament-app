...
function countRepeatPartnerships(
  groups: string[][],
  ...
  for (const group of groups) {
    ...
    const minRepeats = Math.min(
      ...pairings.map(([p1, p2]) => {
        const k1 = partnerKey(p1[0], p1[1]);
        const k2 = partnerKey(p2[0], p2[1]);
        return (partnerCounts.get(k1) ?? 0) + (partnerCounts.get(k2) ?? 0);
      })
    );
    repeats += minRepeats;
  }
...
/** Seed partner counts and games played from existing rounds */
function seedFromRounds(
  existingRounds: Round[],
  players: Player[]
): { partnerCounts: Map<string, number>; gamesPlayed: Map<string, number> } {
  ...
  return { partnerCounts, gamesPlayed };
}
...
function generateRounds(
  ...
  for (let r = 0; r < count; r++) {
    // Select sit-outs: players with most games sit out first
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const score = countRepeatPartnerships(groups, partnerCounts);
      if (score < bestScore) {
        bestScore = score;
        bestGroups = groups;
        if (score === 0) break;
    ...
    const matches: Match[] = bestGroups.map((group, idx) => {
      const pairing = bestPairingForGroup(group, partnerCounts);
    ...
    // Update partner counts and games played
...
export const americanoStrategy: TournamentStrategy = {
  ...
  generateSchedule(players: Player[], config: TournamentConfig): ScheduleResult {
    ...
    const gamesPlayed = new Map<string, number>();
    players.forEach(p => gamesPlayed.set(p.id, 0));
    return generateRounds(players, config, totalRounds, 1, new Map(), gamesPlayed);
  },
  ...
  generateAdditionalRounds(players: Player[], config: TournamentConfig, existingRounds: Round[], count: number, excludePlayerIds?: string[]): ScheduleResult {
    const activePlayers = excludePlayerIds?.length
      ...
      : players;
    const { partnerCounts, gamesPlayed } = seedFromRounds(existingRounds, activePlayers);
    const startRoundNumber = existingRounds.length + 1;
    return generateRounds(activePlayers, config, count, startRoundNumber, partnerCounts, gamesPlayed);
  },
